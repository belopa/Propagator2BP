function [orb,Orbit] = rv2orb(R,V,u)
...
% Outputs: 
%   orb   - The values for all the Keplerian elements          
%   Orbit - Eelements the parameters of the orbit stored 

%% Equations & Settings:
%Angle between two vectors
 ANG = @(A,B) acosd(round(dot(A,B)/(norm(A)*norm(B)),5));
...
%% OTHER: 
  ...
  % Eccentric Anomaly:
      E = acos((e+cosd(Vo))/(1+e*cosd(Vo))); % [rads]
...
%% Output:
        ...
        Orbit.a = a;    Orbit.p = p;
        Orbit.i = i;                        % [degs]
        Orbit.w = w;    Orbit.II = II;      % [degs]
        Orbit.Vo = Vo;  Orbit.uo = uo;      % [degs]
        Orbit.Lo = Lo;  Orbit.T = T;
        Orbit.ra = ra;  Orbit.rp = rp;
        Orbit.E = E;    Orbit.M = M;        % [rads]
        Orbit.TP = TP;  Orbit.MM = MM;
        Orbit.Omega = Omega;                % [degs]
        Orbit.TA = TA;  % [degs]
        Orbit.TA2 = 0;  % [degs]


% This statement basically says: In the first element of the array Pram, if
% you have nothing ('') or a ('a'), replace the first element in the array
% 'orb' with a. If you have p ('p'), replace the first element in the arrat
% 'orb' with p. if it's neither, display an error.
  % Argument 1:
      switch Pram{(1)}
          case {'','a'}
             orb(1) = a;
